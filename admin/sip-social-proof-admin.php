...
define( 'SIP_SPWC_ADMIN_VERSION' , '1.0.3' );
...
class SIP_Social_Proof_WC_Admin {
	...
	public function __construct() {
    	...
    	add_action( 'admin_enqueue_scripts', array( &$this, 'sip_spwc_admin_enqueue') );
	}
	...
	* @since 1.0.0
	*/
	public function sip_spwc_admin_enqueue( ) {

		wp_enqueue_script( 'sip_spwc_admin_script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js' , array('jquery'), '1.0.0', true );
	}
	  	...
	  	if( ! class_exists( 'SIP_Social_Proof_Add_On_WC' ) ) {
	        ...
	        <table style="width:98.5%; margin-top:10px; border-spacing:0px;">
	          <tr class="sip-notification-message">
	            ...
	            <td>
	              <div class="title">Do you want even more <b>Social Proof</b>?</div>
	              <div class="subbtitle">Extend standard plugin functionality with advanced sales, customers and store stats, animated counters and a ton of other features.</div>
	            </td>
	            <td>
	              <div class="button_div">
	                ...
	                <p style="text-align:center;"><span class="dismiss"><?php printf(__('<a href="%1$s">Dismiss</a>'), 'admin.php?page=sip-social-proof-settings&sip_spwc_nag_ignore=0'); ?></span></p>
	              </div>
	            </td>
	          </tr>
	        </table>
	...
	public function sip_spwc_nag_ignore() {
  	...
  	public function sip_spwc_action_links( $links ) {
   		...
   		if( ! class_exists( 'SIP_Social_Proof_Add_On_WC' ) ) {
      		$plugin_links[] = '<a target="_blank" href="' .SIP_SPWC_PLUGIN_PURCHASE_URL. '?utm_source=wordpress.org&utm_medium=SIP-panel&utm_content=v'. SIP_SPWC_PLUGIN_VERSION .'&utm_campaign='.SIP_SPWC_UTM_CAMPAIGN.'" style="color: red; font-weight: bold">' . __( 'Go Pro!', 'sip-social-proof' ) . '</a>';
    	}

    	$plugin_links[] = '<a href="' . admin_url( 'admin.php?page=sip-social-proof-settings' ) . '">' . __( 'Settings', 'sip-social-proof' ) . '</a>';
	...
	*/
	public function sip_spwc_settings_page_ui() { ?>
		...
		<div class="sip-spwc-wrap wrap">
			<h2>SIP Social Proof WooCommerce</h2>
		  	<div class="sip-container">
				<h2 class="nav-tab-wrapper">
				  ...
				  <?php if( class_exists( 'SIP_Social_Proof_Add_On_WC' ) ) { ?>
						<a class="nav-tab<?php if ( isset( $_GET['action'] ) && 'licenses' == $_GET['action'] ) echo ' nav-tab-active'; ?>" href="admin.php?page=sip-social-proof-settings&amp;action=licenses"><?php _e( 'Licenses', 'sip-social-proof' ); ?></a>
					<?php } ?>
				...
				<?php
					...
					} elseif ( 'licenses' == $_GET['action'] ) {
						include( "ui/licenses.php");
					}
			  ?>
			</div><!-- .container -->
		</div><!-- .sip-sp-wrap -->
